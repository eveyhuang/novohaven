...
exports.getUserInputVariables = getUserInputVariables;
exports.compilePrompt = compilePrompt;
exports.getRequiredInputsForRecipe = getRequiredInputsForRecipe;
exports.validateInputs = validateInputs;
const database_1 = require("../models/database");
// Regex to match variables in format {{variable_name}}
const VARIABLE_REGEX = /\{\{([^}]+)\}\}/g;
// Extract all variables from a prompt template
function extractVariables(promptTemplate) {
    const variables = [];
    let match;
...
function isCompanyStandardVariable(varName) {
    return COMPANY_STANDARD_VARIABLES.some(standard => varName.toLowerCase().includes(standard.toLowerCase().replace(/_/g, ''))
...
function getUserInputVariables(promptTemplate) {
...
function resolveCompanyStandard(varName, userId) {
    ...
    const standards = database_1.queries.getStandardsByType(userId, matchedType);
...
function formatStandardContent(content, type) {
    const lines = [];
    switch (type) {
        case 'voice':
            if (content.tone)
                lines.push(`Tone: ${content.tone}`);
            if (content.style)
                lines.push(`Style: ${content.style}`);
            if (content.guidelines && content.guidelines.length > 0) {
                lines.push('Guidelines:');
                content.guidelines.forEach((g) => lines.push(`- ${g}`));
            }
            break;
        case 'platform':
            if (content.platform)
            ...
            if (content.requirements && content.requirements.length > 0) {
                ...
                content.requirements.forEach((r) => lines.push(`- ${r}`));
            }
        ...
        case 'image':
            ...
            if (content.dimensions)
                lines.push(`Dimensions: ${content.dimensions}`);
            if (content.guidelines && content.guidelines.length > 0) {
                lines.push('Guidelines:');
                content.guidelines.forEach((g) => lines.push(`- ${g}`));
            }
...
function resolvePreviousStepOutput(stepNumber, stepExecutions) {
...
function extractMediaType(base64) {
...
function compilePrompt(promptTemplate, context) {
    const variables = extractVariables(promptTemplate);
        ...
        switch (variable.type) {
            ...
            case 'previous_step':
                if (variable.stepNumber !== undefined) {
                    resolvedValue = resolvePreviousStepOutput(variable.stepNumber, context.stepExecutions);
                }
                else {
                    unresolvedVariables.push(variable.name);
...
function getRequiredInputsForRecipe(recipeId) {
...
function validateInputs(recipeId, providedInputs) {
    const required = getRequiredInputsForRecipe(recipeId);
    const missing = required.filter(input => !providedInputs[input] || providedInputs[input].trim() === '');
    return {
