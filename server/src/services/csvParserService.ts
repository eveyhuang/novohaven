...
function parseRating(value: string): number {
  ...
  if (starsMatch) return parseFloat(starsMatch[1]);

  // Handle plain number
  const num = parseFloat(value);
  if (!isNaN(num)) {
    // If number is greater than 5, assume it's out of 10 or 100
    if (num > 5 && num <= 10) return (num / 10) * 5;
    if (num > 10 && num <= 100) return (num / 100) * 5;
...
export function parseReviewCSV(
  content: string,
  platform: ScrapingPlatform = 'amazon',
  productUrl?: string
): CSVParseResult {
...
export function reviewsToCSV(reviews: ReviewData[]): string {
...
export function reviewsToJSON(reviews: ReviewData[]): string {
